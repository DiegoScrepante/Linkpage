https://diegoscrepante.github.io/Linkpage/

🔗 Linkpage

Uma página de links personalizada inspirada no conceito Link-in-bio, que reúne redes sociais, portfólio e outros links importantes em um único lugar.

O projeto foi desenvolvido com foco em prática de desenvolvimento front-end e criação de páginas simples, responsivas e fáceis de compartilhar.

🚀 Funcionalidades

📎 Centralização de links importantes

👤 Exibição de perfil e redes sociais

📱 Layout responsivo para mobile e desktop

🎨 Interface simples e personalizável

⚡ Carregamento rápido por ser uma página estática

🛠️ Tecnologias Utilizadas

HTML

CSS

JavaScript

📷 Preview

![Screenshot_11](https://github.com/DiegoScrepante/Linkpage/assets/126990879/0749f0da-e4d1-4f78-b65a-2dc5ae225e4a)
![Screenshot_12](https://github.com/DiegoScrepante/Linkpage/assets/126990879/5c7bfa13-84d6-4567-b43f-6d583375c73b)


▶️ Como usar

1️⃣ Clonar o repositório

git clone https://github.com/DiegoScrepante/Linkpage.git

2️⃣ Abrir o projeto

Abra o arquivo:

index.html

no navegador.

🎯 Objetivo do Projeto

Este projeto foi criado com fins educacionais e de portfólio, demonstrando habilidades em desenvolvimento web e criação de interfaces simples para compartilhamento de links.

👨‍💻 Autor

Diego Screpante

GitHub
https://github.com/DiegoScrepante

<div align="center">
  <small>Diego Aparecido Screpante - 2023</small>
</div>
